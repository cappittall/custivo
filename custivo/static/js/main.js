...
const wristModesCommonSettings = {
  ...
  occluderQuaternion: [0.707,0,0,0.207], // rotation of Math.PI/2 along X axis,
  occluderFlattenCoeff: 1.0, // 1 -> occluder is a cylinder 0.5 -> flatten by 50%
  occluderModelURL: 'assets/occluders/wristOccluder.glb',
...
const ringModesCommonSettings = {
  threshold: 0.80, // detection sensitivity, between 0 and 1
...
const ringModelCommonSettings = {
  URL: 'assets/glbs/yuzuk/yuzukUst.glb',
...
function setup_lighting(three){
  ...
  // TODO: customize
  const hemiLight = new THREE.HemisphereLight( 0xffffff, 0x000000, 500 );
  scene.add(hemiLight);
...
function load_model(modelId, threeLoadingManager){
  ...
  new THREE.GLTFLoader(threeLoadingManager).load(modelSettings.URL, function(model){
    console.log('LOADINFO in main.265', model.scene.children);
    const me = model.scene;
    me.scale.set(30, 30, 30);
    ...
    if (modelSettings.quaternion){
      ...
      me.quaternion.set(q[0], q[1], -q[2], q[3]);
    }
...
function add_hardOccluder(VTOModeSettings){
  return new Promise(function(accept, reject){
    new THREE.GLTFLoader().load(VTOModeSettings.occluderModelURL, function(model){
      console.log('LOADINFO in main.353');
